...
def update_question(question, qid):
    print(f"update question...{question['id']}")
    ...
    elif STACKS.get(question["sub_type"], question["sub_type"]) == question["sub_type"]:
    ...
    payload = json.dumps({
        ...
        "sub_type": STACKS.get(question["sub_type"], question["sub_type"])
    })
...
def process_validator_response(validator_response):
    ...
    if not validator_response['valid']:
        for step, value in validator_response['data'].items():
            if value['valid'] != True and value['valid'] != 'skip':
                print(step, value)
